...
void File::readData(const std::shared_ptr<char[]>& data, size_t bytesCount) const {
  ...
  size_t i = 0;
  //    for each block
  for (std::shared_ptr<Block> thisBlock(this->firstBlock.get()); thisBlock != nullptr; thisBlock = std::move(thisBlock->next)) {
    //      for each file data byte
    for (size_t j = 0; j < blockSize && i < bytesCount; ++j, ++i) {
      data.get()[i] = thisBlock->data.get()[j];
    }
  }
}

void File::writeData(const std::shared_ptr<char[]>& data, size_t bytesCount) const {
  ...
  for (std::shared_ptr<Block> thisBlock(this->firstBlock.get()); thisBlock != nullptr; thisBlock = std::move(thisBlock->next)) {
    //      for each file data byte
    for (size_t j = 0; j < blockSize && i < bytesCount; ++j, ++i) {
      thisBlock->data.get()[j] = data.get()[i];
    }
  }
}
